...
Player parse_player(const std::string& value) {
    if (value == "black" || value == "Black" || value == "B") {
        return Player::Black;
    }
    if (value == "white" || value == "White" || value == "W") {
        return Player::White;
    }
...
Cell parse_cell(char value) {
    ...
    if (value == 'B' || value == 'b' || value == '1') {
        return Cell::Black;
    }
    if (value == 'W' || value == 'w' || value == '2') {
        return Cell::White;
    }
...
CreatorPosition load_creator_position(const std::string& path) {
    ...
    if (!input) {
        throw std::runtime_error("failed to open position file: " + path);
    }
    ...
    if (!input || size < 4 || size % 2 != 0) {
        throw std::runtime_error("invalid position header");
    }
    ...
    for (std::size_t row = 0; row < size; ++row) {
        ...
        input >> line;
        if (line.size() != size) {
            throw std::runtime_error("invalid board row length");
        }
...
std::vector<AICreatorRunResult> compare_creator_ais(
    ...
    for (auto& package : packages) {
        results.push_back(run_creator_inference(package, position));
    }
    return results;
}
